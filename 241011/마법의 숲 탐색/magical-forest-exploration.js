    ...
    이동 방법 1이 가능한지 확인
        ...
        불가능하면 이동방법 2가 가능한지 확인
            ...
            불가능하면 이동 방법 3이 가능한지 확인
                ...
                불가능하면 
                    1) 골렘 몸이 숲 밖에 있는지 확인 후, 그렇다면 모든 골렘 삭제 후 다음 정령&골렘으로 넘어감. 그렇지 않다면 2번으로
                    2) 정령 이동 실행.
...
rl.on("close", () => {
    ...
    const height = R + 3;
    const width = C;
    const forest = Array.from({length: height}, () => Array.from({length: width}, () => [0, false]));

    // 골렘 중앙 위치로부터 골렘이 중앙, 북, 동, 남, 서 차지하고 있는 위치 반환
    function getCurGolemSpace(curPosition) {
        const [curY, curX] = curPosition;
        return [[curY, curX], [curY-1, curX], [curY, curX+1], [curY+1, curX], [curY, curX-1]]
    }

    function isMove1Available(curPosition) {
        const [curCenter, curNorth, curEast, curSouth, curWest] = getCurGolemSpace(curPosition);
        if (
            (curWest[0] + 1 >= 0 && curWest[0] + 1 < height && forest[curWest[0] + 1][curWest[1]][0] === 0) && 
            (curSouth[0] + 1 >= 0 && curSouth[0] + 1 < height && forest[curSouth[0] + 1][curSouth[1]][0] === 0) &&
            (curEast[0] + 1 >= 0 && curEast[0] + 1 < height && forest[curEast[0] + 1][curEast[1]][0] === 0)
            ) {
            return true;
        }
    ...
    function isMove2Available(curPosition) {
        const [curCenter, curNorth, curEast, curSouth, curWest] = getCurGolemSpace(curPosition);
        if (
            (curNorth[1] - 1 >= 0 && curNorth[1] - 1 < width && forest[curNorth[0]][curNorth[1] - 1][0] === 0) &&
            (curWest[1] - 1 >= 0 && curWest[1] - 1 < width && forest[curWest[0]][curWest[1] - 1][0] === 0) &&
            (curSouth[1] - 1 >= 0 && curSouth[1] - 1 < width && forest[curSouth[0]][curSouth[1] - 1][0] === 0) &&
            (curWest[0] + 1 >= 0 && curWest[0] + 1 < height && forest[curWest[0] + 1][curWest[1] - 1][0] === 0) &&
            (curSouth[0] + 1 >= 0 && curSouth[0] + 1 < height && forest[curSouth[0] + 1][curSouth[1] - 1][0] === 0)
            ) {
            return true;
        }
    ...
    function isMove3Available(curPosition) {
        ...
        if (
            (curNorth[1] + 1 >= 0 && curNorth[1] + 1 < width && forest[curNorth[0]][curNorth[1] + 1][0] === 0) &&
            (curEast[1] + 1 >= 0 && curEast[1] + 1 < width && forest[curEast[0]][curEast[1] + 1][0] === 0) &&
            (curSouth[1] + 1 >= 0 && curSouth[1] + 1 < width && forest[curSouth[0]][curSouth[1] + 1][0] === 0) &&
            (curEast[0] + 1 >= 0 && curEast[0] + 1 < height && forest[curEast[0] + 1][curEast[1] + 1][0] === 0) &&
            (curSouth[0] + 1 >= 0 && curSouth[0] + 1 < height && forest[curSouth[0] + 1][curSouth[1] + 1][0] === 0)
            ) {
            return true;
        }
